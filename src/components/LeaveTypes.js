...
function LeaveTypes() {
    ...
    const handleEditBtn = async (leaveType) => {
        try {
            setSelectedLeaveTypes(leaveType);
            setFormMode("edit");
            
            // Set form values for editing
            setValue("leave_type_code", leaveType.leave_type_code || "");
            setValue("leave_type_name", leaveType.leave_type_name || "");
            setValue("status", leaveType.status === 'Active' ? "1" : "0");
            setValue("leave_type_id", leaveType.leave_type_id || "");
    ...
    const onSubmit = async (data) => {
        ...
        try {
            if (formMode === 'edit') {
                ...
                await axios.post('/api/leave_type', data, {
                    headers: {
                        'userId': userId
                    }
                });
    ...
    return (
        <div className="table-container" style={{ overflowX: 'hidden', overflowY: 'hidden', width: '100%', padding: '30px' }}>
            ...
            <div className="offcanvas offcanvas-end customeoff addtask" tabIndex="-1" id="offcanvasExample1">
                ...
                <div className="offcanvas-body">
                    <div className="container-fluid">
                        <form id="leave_type-create" className="row g-3 needs-validation" onSubmit={handleSubmit(onSubmit)} encType="multipart/form-data">
                            <div className="col-md-12">
                                <label htmlFor="leave_type_code" className="form-label">Leave Type Code <span className='text-danger'>*</span></label>
                                <input
                                    type="text"
                                    className={`form-control ${errors.leave_type_code ? 'is-invalid' : ''}`}
                                    id="leave_type_code"
                                    placeholder="Leave Type Code"
                                    {...register("leave_type_code", { required: "Leave type code cannot be empty" })}
                                />
                                {errors.leave_type_code && <span className="text-danger">{errors.leave_type_code.message}</span>}
                            </div>
                            <div className="col-md-12">
                                <label htmlFor="leave_type_name" className="form-label">Leave Type Name <span className='text-danger'>*</span></label>
                                <input
                                    type="text"
                                    className={`form-control ${errors.leave_type_name ? 'is-invalid' : ''}`}
                                    id="leave_type_name"
                                    placeholder="Leave Type Name"
                                    {...register("leave_type_name", { required: "Leave type name cannot be empty" })}
                                />
                                {errors.leave_type_name && <span className="text-danger">{errors.leave_type_name.message}</span>}
                            </div>
                            <div className="col-md-12">
                                <div className="custom__form__inputs">
                                    <label htmlFor="status" className="form-label">Status <span className='text-danger'>*</span></label>
                                    <div className="">
                                        <select 
                                            ...
                                            id="status"
                                            {...register("status", { required: "Status cannot be empty" })}
                                        >
                                            <option value="">Select</option>
                                            <option value="1">Active</option>
                                            <option value="0">Inactive</option>
                            ...
                            <input type="hidden" {...register("leave_type_id")} />
                        </form>
                    </div>
                </div>
                <div className="mb-3">
                    <div className="d-md-flex d-flex d-grid align-items-center gap-3 justify-content-end">
                        <button type="submit" form="leave_type-create" className="btn btn-primary px-4 template-btn">{formMode === 'edit' ? 'Update' : 'Add'}</button>
                        <button className="btn btn-danger px-4" type="button" data-bs-dismiss="offcanvas" aria-label="Close" onClick={handleCancel}>Cancel</button>
                    </div>
                </div>
            </div>
